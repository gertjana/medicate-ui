...
type alias CombinedMedicine = (Medicine, Float)
combinedMedicineDecoder: Decoder CombinedMedicine
combinedMedicineDecoder = Decode.map2 Tuple.pair
    (Decode.index 0 medicineDecoder)
    (Decode.index 1 Decode.float)

type alias CombinedMedicines = List CombinedMedicine
combinedMedicinesDecoder : Decoder CombinedMedicines
combinedMedicinesDecoder = Decode.list combinedMedicineDecoder
...
type alias CombinedSchedules = List CombinedSchedule
combinedSchedulesDecoder : Decoder CombinedSchedules
combinedSchedulesDecoder = Decode.list combinedScheduleDecoder

viewCombinedMedicine: (Medicine, Float) -> Html msg
viewCombinedMedicine (medicine, amount) = li [] 
...
viewCombinedMedicines cms = ul [] (List.map (\cm -> viewCombinedMedicine cm) cms)

viewCombinedSchedule: CombinedSchedule -> Html msg
viewCombinedSchedule cs = tr []
                        [ td [] [ text cs.time]
                        , td [class "left"] [ viewCombinedMedicines cs.medicines]
                        ]

viewCombinedSchedules: CombinedSchedules -> Html msg
viewCombinedSchedules css = table [ class "combined table table-striped table-condensed table-hover table-bordered" ]
                    [ 
